...
[문제]
    ...
    COMM이 널인 사원들의 정보를 보여주세요
    
    SELECT * FROM EMP
    WHERE COMM =NULL; [X]
    ==> 데이터가 안나옴
    
    NULL값 여부를 판단할때는 '='를 사용하면 안된다
    IS NULL/ IS NOT NULL 을 이용해야 한다
...
--------------------------------------------------------       
	 ...
	 4]	상품테이블에서 공급업체별(EP_CODE_FK)로 평균판매가 AVG(컬럼명)를 구하되 
		평균판매가 오름차순으로 보여주세요.
